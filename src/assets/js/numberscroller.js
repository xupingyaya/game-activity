export default function(_config){
    ...
    initNumberScroll=function(container,unscroll){//unscroll参数表示只是显示数字，不滚动
        ...
        if(!diffrentiation){
            ...
            if(undreg.indexOf("^")==0){
                ...
                for(let i=len-1;i>=0&&undregArr.length;--i){
                    if(undregArr.pop()=='x'){
    ...
    createScrollNum=function(start){
        ...
        var str='';
        start = start || 0;
        for(let i=start, len = config.numList.length;i<len;++i){
    ...
    createScrollNum2=function(start, hasLasterNum){
        ...
        var str='';
        start = start || 0;
        for(let i=config.numList.length-1; i>=start; --i){
    ...
    createNum=function(number,oldNumber,groupIndex,direction){
        ...
        if('up'==direction){ console.log("up");
            ...
            if(typeof(oldNumber) == "number") {
                ...
                // 占位符
                str+='<div style="position:relative; width:100%; height: '+h+'px; line-height: '+h+'px; overflow:hidden; padding: 0; margin: 0; left: 0; top: 0;">'+config.numList[oldNumber]+'</div>';
            }
            ...
            if(typeof(oldNumber) == "number") {
                ...
                str+='<div style="position:relative; width:100%; height: '+h+'px; line-height: '+h+'px; overflow:hidden; padding: 0; margin: 0 0 -'+h+'px 0; left: 0; top: 0;">'+ config.numList[oldNumber] +'</div>';
            }
    ...
    set=function(_config){
        ...
        for(var i in config){
            console.log(123);
            if (Object.getOwnPropertyDescriptor(_config, i)) {
                config[i]=_config[i];
            }
