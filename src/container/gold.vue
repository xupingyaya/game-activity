<template>
  <div class="home" :style="{ height: Height+'px'}">
   ...
   <div  id='test' :style="{'display': display }">
          <img  v-for="index in randomCoinCount" :key="index"   :id="'test'+index" src="../assets/images/gold/smgold.png" style='height:42px;width:42px' alt="">
    </div>
...
export default {
  ...
  methods: {
    ...
    // {"x":177,"y":249,"z":20,"rotate":138,"height":0},{"x":177,"y":216,"z":20,"rotate":138,"height":0},{"x":177,"y":183,"z":20,"rotate":138,"height":0},
    initIcon(){
      const _returnObj =[{"x":27,"y":88,"z":20,"rotate":138,"height":0},{"x":117,"y":174,"z":20,"rotate":138,"height":0},{"x":101,"y":110,"z":20,"rotate":138,"height":0},{"x":57,"y":148,"z":20,"rotate":138,"height":0},{"x":17,"y":128,"z":20,"rotate":138,"height":0},{"x":17,"y":173,"z":20,"rotate":138,"height":0},{"x":-20,"y":170,"z":20,"rotate":138,"height":0},{"x":177,"y":126,"z":20,"rotate":138,"height":0},{"x":1,"y":52,"z":20,"rotate":138,"height":0},{"x":107,"y":52,"z":20,"rotate":138,"height":0},{"x":67,"y":58,"z":20,"rotate":138,"height":0},{"x":165,"y":42,"z":20,"rotate":138,"height":0},{"x":177,"y":168,"z":20,"rotate":138,"height":0},{"x":177,"y":82,"z":20,"rotate":138,"height":0},{"x":60,"y":190,"z":20,"rotate":270,"height":0}]
      let div1=window.innerWidth;
      if(div1 >400){
        _returnObj.push({"x":117,"y":258,"z":20,"rotate":138,"height":0})
      }
      if(div1 > 375){
        _returnObj.push({"x":203,"y":50,"z":20,"rotate":323,"height":0})
        _returnObj.push({"x":117,"y":216,"z":20,"rotate":138,"height":0})
      }
    let imgtest=''
       _returnObj.forEach((item)=>{
          let ystart= item.y
          let  xstart =item.x
          imgtest = imgtest + `<img class="gold-item" src='${gold}' style='top: ${ystart}px;left: ${xstart}px;width:42px; height:42px'/>`;
      })
    ...
    newFun(_returnObj1,arr){
      ...
      _returnObj1.forEach((item)=>{
              ...
              if((len === 0 || len<42 && len >38) && Math.abs(ySpan) > Math.abs(xSpan)){
                  flag =true
                  }
              })
                if(flag){
                     _returnObj2.push({x: item.x, y: item.y+1, rotate:item.rotate, height: item.height,flag:true});
                    ystart= item.y+2
                    xstart =item.x
                }else{
   ...
   // 当前游戏进行中，不可以点击
    ...
    // 实现抛物线的效果
    throwCoin(){
      if (this.randomCoinCount > 1) {
        createjs.Sound.play("goldFallMusic");
      } else {
        createjs.Sound.play("multGoldFallMusic");
      }
      console.log('randomCoinCount---333', this.randomCoinCount)
      var $ball = document.getElementById("test").getElementsByTagName("img")
            let arr =[]
            if(this.randomCoinCount === 1){
                arr.push({x: 107, y: 48})
            }
            if(this.randomCoinCount === 2){
                 arr.push({x: 153, y: 48})
                arr.push({x: 107, y: 48})
            }
          if(this.randomCoinCount === 3){
              arr.push({x: 153, y: 48})
              arr.push({x: 107, y: 48})
              arr.push({x:47,y:48})
            }
       console.log('randomCoinCount---88', $ball)
      createjs.Sound.play("coinClashMusic");
